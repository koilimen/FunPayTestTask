...
Этот репозиторий содержит решение тестового задания на позицию ведущего php разработчика.

## Примечания разработчика.

Для реализации исходного задания добавлено несколько файлов:

```ParameterInterface.php``` - интерфейс класса параметра.

```*Parameter.php``` - реализуют конвертацию пареметра в строковое представление.

```ParamsFactory.php``` - фабрика для создания объекта класса параметра по спецификатору.

```unit_tests.php``` - набор собственных юнит тестов.
