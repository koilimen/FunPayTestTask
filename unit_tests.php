...
$mysqli = @new mysqli('localhost', 'admin', 'password', 'database', 3306);
...
$testData = [
    ...
    ['SELECT * FROM foo.bar WHERE id = 5.5 AND name = \'Alex\'', ['SELECT * FROM foo.bar WHERE id = ? {AND name = ?}', [5.5, 'Alex']]],
    ['SELECT * FROM foo.bar WHERE id = 5.5 AND surname = \'Bill\'', ['SELECT * FROM foo.bar WHERE id = ? {AND surname = ?}', [5.5, 'Bill']]],
